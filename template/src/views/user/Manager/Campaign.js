...
const UserCampaign = () => {
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            ...
            //let selected_campaign_group = newSelectedId.join(','); 
            //let selected_campaign_name = nameCampaign.join(','); 
            setTotalCampaign(newSelectedId.length) 
    ...
    const handleClick = (event, id, name) => { 
        ...
        if (selectedIndex === -1) {
            ...
            nameCampaign = nameCampaign.concat(selected, name);
        ...
        var count = -1;
        // storeAllData.forEach(function(item){
        //     count++;
        ...
        // const newSelectedId = rows.map((n) => n.id); 
        // const newSelectedName = rows.map((n) => n.name); 
        let selected_campaign_group = newSelected.join(','); 
        let selected_campaign_name = nameCampaign.join(','); 
        setTotalCampaign(newSelected.length) 
        setUtmCampaign(selected_campaign_group);
        setUtmCampaignName(selected_campaign_name);
        if(selected_campaign_group != ''){
    ...
    const searchStatus = (e) => {  
        ...
        axios.get(`${API_PATH}/api/linkedin/campaign/${campaign_group_id}-${localStorage.getItem('user_token')}-${search_date_range}--`).then((response) => {
    ...
    function updateUtmParameter(){  
        ...
        axios.post(`${API_PATH}/api/linkedin/updateCampaignUtm`,send_data).then((response) => {
            setBasicModal(false)
            ...
            const update_interval = setInterval(function(){ 
                //console.log(`${API_PATH}/api/getAdsUpdated/${localStorage.getItem('user_token')}`)
                axios.get(`${API_PATH}/api/getAdsUpdated/${localStorage.getItem('user_token')}`).then((response) => { 
                    count += 1;  
                    //console.log(count)
                    if(count == 1){ 
                        $('#ads_update').show();
    ...
    const addUtmParameter = ()=>{ 
        ...
        const regex = /\s*,\s*/; // regular expression to match comma with optional whitespace
        
        if (utmCampaign == 0 || utmCampaign == '') {
            alert('Please select compaign.');   return false;
        }

        const comparray = utmCampaign.split(regex);
    ...
    const selectUtm = (e)=>{  
        ...
        $('#utm_name_id').val(id);
        //console.log(`${API_PATH}/api/user/utm/${id}`); 
    ...
    $(document).on('keyup change','.utm_value, .utm_parameter', function(e){  
        ...
        $('.sample_parameter_code').val(`?${code_string}`);
        $('#demo_text').val(`?${code_string}`); 
        //$('.sample_parameter_code').attr('value', 'new value');
        setTextParameter(`?${code_string}`) 
    });
    ...
    async function getUtmData(){
        ...
        //console.log(`${API_PATH}/api/user/utmParameter/${account_id_for_utm}-${localStorage.getItem('user_token')}`);
    ...
    async function getData(search_date_range='',filter_data='',is_search_val_exist=''){  
        ...
        //console.log(`${API_PATH}/api/linkedin/campaign/${campaign_id}-${localStorage.getItem('user_token')}-${search_date_range}-${filter_data}`)
        await axios.get(`${API_PATH}/api/linkedin/campaign/${campaign_id}-${localStorage.getItem('user_token')}-${search_date_range}-${filter_data}`).then((response) => { 
    ...
    useEffect(()=>{ 
        ...
        // const timeout = setTimeout(() => { 
        //    alert(1)
        //     $(`.parameter_value_1`).append(`<input type="text" name="parameter_value[@0|insert@]" className="form-control utm_value account_name" value="" disabled />`) 
        // }, 7000);
        ...
        <MainCard> 
                            ...
                            <div className="container col-md-12 text-wrap">
                                ...
                                <div className="col-md-12">
                                    <div className="para_save_outer pull-right">
                                        ...
                                        <button className="utm_btn" onClick={updateUtmParameter}  disabled={ utmParameter.length == 0 ? true : utmCampaignName === '' ? true : false}>Apply UTM</button> 
                                    </div>
                                </div> 
                                </div>
                                <div className="row">
                        ...
                        {/* <div className="modal-body"> */}
                            <form action="" method="POST" onSubmit={handleSubmit(addUtmParameter)} autoComplete="OFF" noValidate>
                                <div className="row" id='parameter_form'> 
                                    ...
                                    <div className="row hide_retrieve_parameter" id="add_parameter_div_1">
                                    ...
                                    <div class="" id="retrieve_parameter"></div> 
                                    <div class="" id="add_more"></div>
                                ...
                                <div className="row">
                                    <input class="input_cus" type="hidden" id="utm_name_id" />
                                    <div className="col-md-12">
            ...
            {/* <div className="ads_update_design" id="ads_update" style={{ display:'none'}}></div> */}
                <Grid item xs={12} md={6}>
                    ...
                    <Tab component={Link} to="#" icon={<CampaignIcon sx={{ fontSize: '1.3rem' }} />} label='Campaign' />
                    ...
                    {/* <span variant="contained" type="button" className="open_account open_account_wigard utmBTN" onClick={toggleShow}> 
                                    ...
                                    rows != "undefined" && stableSort(rows, getComparator(order, orderBy))
                                        ...
                                        .map((row, index) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    <TableCell padding="checkbox" sx={{ pl: 3 }} onClick={(event) => handleClick(event, row.id, row.name)}>
                                                        <Checkbox
                                                            color="primary"

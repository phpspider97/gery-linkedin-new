...
//     { field: 'adtypes', headerName: 'Top 10 performing landing', width: 350, align:'left',headerAlign: 'left', sortable: false },
...
const columns = [ 
    ...
    { field: 'adtypes', headerName: 'Top 10 performing landing', width: 250, align:'left',headerAlign: 'left', sortable: true },
...
export default function TableDataGrid(props) {
    ...
    const selectAdsAccound = async (props)=>{ 
        ...
        //console.log('11__',`${API_PATH}/api/linkedin/adsLanding/${localStorage.getItem('select_account')}-${localStorage.getItem('user_token')}-${search_date_range}-${filter_data}`)
        await axios.get(`${API_PATH}/api/linkedin/adsLanding/${localStorage.getItem('select_account')}-${localStorage.getItem('user_token')}-${search_date_range}-${filter_data}`).then((response) => {
            //var account_data = []
            var rows = []; 
            var count = -1;
                ...
                if(get_val != 'NA'){
                    ...
                    if( count<10  ){
                        rows[count] = { 
                            id: count+1,
                            adtypes: get_val,
                            // adtypes: response.data[get_val].ad_title,
                            ...
                            results: response.data[get_val].result.toLocaleString("en-US"),

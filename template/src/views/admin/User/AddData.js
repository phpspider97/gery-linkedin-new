...
const AddData = ({ open, handleCloseDialog, getStaffData }) => {
    ...
    const submitData = (e)=>{ 
        ...
        if(id){ 
            axios.put(`${API_PATH}/api/user/${id}`, sendData).then((response) => { 
            ...
            axios.post(`${API_PATH}/api/user`, sendData).then((response) => { 
                if(response.status == 200){  
                    toast(`${response.data.msg}`) 
                ...
                <>
                    ...
                    <DialogContent style={{height:'610px'}}>
                        <Grid container spacing={gridSpacing} sx={{ mt: 0.25 }}>
                            ...
                            <Grid item xs={12}>
                                <TextField fullWidth label="Password*" defaultValue=" " type="password" id="user_password" {...register('user_password', { required: 'Password is required.', minLength: {
                                    value: 8,
